...
7. **Reading & learning** — synthetic fixture. Mateo has a posted spring English A- and a current term with no grade. Willow Bend Reading Check (fictional 1–4) is meeting then exceeding on the grade 4 form. Checkout does not change grades.
...
1. Open My day. Show Tyler as in progress and Jordan as upcoming, then review Needs attention. Start now on a booked conversation if you want to jump in before the slot.
...
6. Open Reading changes with Illustrative history. The default eight-student cohort has six eligible pairs and two exclusions across borrowing, English and reading checks. Select a chart category to inspect matching student evidence, then Show all students to restore the cohort. Filter to Tom: Sofia does not transfer attribution merely because a later shared contact exists. Switch to Live session: no historical evidence is borrowed.
...
Leave `SHELFMATE_LLM` unset. Recs still return. If you enable `SHELFMATE_LLM=on` without `LLM_BASE`/`ANTHROPIC_BASE_URL`, explain mode is `fallback` and ranking is unchanged. Live model calls are not part of the offline rehearsal.
