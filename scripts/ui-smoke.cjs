...
(async () => {
  ...
  try {
    ...
    const ready = await fetch(base + '/api/test/clock', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ now: '2026-09-11T16:10:00Z' }) });
    assert.equal(ready.status, 200);
    await page.locator('[data-view=day]').click();
    await page.locator('#agenda-body input[type=date]').fill('2026-09-11');
